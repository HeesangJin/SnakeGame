...
- `index.html`: Single-page shell, embedded styles, and module script loading.
- `snake/app.js`: UI wiring, input handling, render loop, and overlays.
- `snake/gameLogic.js`: Core game state and pure game mechanics.
- `snake/spaceBackground.js`: Canvas-based animated background.
- `snake/TEST_PLAN.md`: Manual UI/interaction checklist.
- `snake/UI_DESIGN.md`, `snake/IMPROVEMENT_PLAN.md`: Design and technical reference docs.
- `snake/artifacts/`: Reference screenshots for visual regression checks.
...
- Keep game logic in `snake/gameLogic.js`; avoid mixing DOM operations into logic functions.
...
- Use `snake/TEST_PLAN.md` as the baseline regression checklist.
- Verify at minimum:
...
- When UI changes are made, compare against `snake/artifacts/` screenshots.
